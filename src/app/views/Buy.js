...
class Buy extends Component {
  ...
  amountProductHandler(e) {
    this.setState({ amountProduct: e.target.value });
  }
    ...
    if (this.state._idProduct) {
      fetch(`/api/oneMove/${this.state._idProduct}`, {
        ...
        body: JSON.stringify({
          ...
          amount: this.state.amountProduct,
  ...
  deleteBuy(id) {
    if (confirm('¿Seguro que desea cancelar la Compra?')) {
      fetch(`/api/move/${id}`, {
        method: 'DELETE',
        headers: {
          'Accept': 'application/json',
  ...
  nullStateProduct() {
    this.setState({ nameProduct: '', amountProduct: '', quantityProduct: '', _idProduct: '' });
  }

  about() {
    if (this.state._id) {
      this.deleteBuy(this.state._id);
    } else {
      this.nullState();
    }
  }
        ...
        <div className="container" id="table-buy" >
          <div className="section">
            <Panel bsStyle="primary">
              <Panel.Heading>
                <Panel.Title componentClass="h3">Nueva Compra</Panel.Title>
              </Panel.Heading>
              <Panel.Body>
                <Form inline>
              ...
              <Panel.Body>
                ...
                <Form inline>
                  ...
                  <FormGroup controlId="formPrice">
                    ...
                    <FormControl type="number" min="0" step="0.1" value={this.state.amountProduct} onChange={(e) => this.amountProductHandler(e)} required />
                  </FormGroup>{' '}
